#!/usr/bin/env bash
#
# Agent Factory — macOS Startup
# Inicia Ollama (GPU/Metal otimizado) + Dashboard.
#
# Uso:
#   ./scripts/start_agent_factory_macos.sh                  # normal
#   ./scripts/start_agent_factory_macos.sh --demo           # + demo agents
#   ./scripts/start_agent_factory_macos.sh --no-ollama      # não gerencia Ollama
#   ./scripts/start_agent_factory_macos.sh --port 9090      # porta customizada
#   ./scripts/start_agent_factory_macos.sh --open           # abre browser
#
...
if [ "$NO_OLLAMA" = false ]; then
    ...
    # macOS-specific: Metal GPU acceleration
    ...
    if [ "$METAL_FLAG" = "1" ]; then
        export GGML_METAL_GPU=1
        echo "  🍎 GPU Acceleration: Metal (Apple Silicon)"
    fi
    ...
    if curl -sf http://localhost:11434/api/tags > /dev/null 2>&1; then
        ...
        if ! command -v ollama &> /dev/null; then
            echo "  ❌ Ollama não encontrado. Instale: brew install ollama"
            echo "     Ou baixe de: https://ollama.com"
            exit 1
        fi
...
DASHBOARD_URL="http://localhost:$PORT/?project=agent-factory-dev"
echo "  📊 $DASHBOARD_URL"
